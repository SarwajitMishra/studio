...
const formSchema = z.object({
  ...
  layoutRating: z.number().min(1, "Rating is required.").max(5),
  uiFeelRating: z.number().min(1, "Rating is required.").max(5),
  overallRating: z.number().min(1, "Rating is required.").max(5),
    ...
    <div className="max-w-4xl mx-auto space-y-8">
        <Card className="shadow-lg text-center">
            <CardHeader>
                ...
                <CardTitle className="text-3xl font-bold text-primary">Feedback & Survey</CardTitle>
                <CardDescription>
                    Your thoughts help us grow! Please take a moment to share your experience with us.
                </CardDescription>
            </CardHeader>
        </Card>
                            ...
                            <div key={game.id} className="p-4 border rounded-lg space-y-3">
                                ...
                                <div className="space-y-3 pl-2 border-l-2 ml-2">
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Textarea placeholder="Any comments on this game?" {...field} value={field.value || ''}/>
                                            )}
                                        />
                                </div>
                                )}
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                     ...
                     {errors.layoutRating && <p className="text-sm text-destructive text-right">{errors.layoutRating.message}</p>}
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                     ...
                     {errors.uiFeelRating && <p className="text-sm text-destructive text-right">{errors.uiFeelRating.message}</p>}
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                     ...
                     {errors.overallRating && <p className="text-sm text-destructive text-right">{errors.overallRating.message}</p>}
                    <Separator />
                    <div>
                        ...
                        />
                        {errors.overallComment && <p className="text-sm text-destructive mt-1">{errors.overallComment.message}</p>}
                    </div>
                </CardContent>
            </Card>
