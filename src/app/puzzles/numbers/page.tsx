...
const QUESTIONS_PER_ROUND = 5; // Used for Arithmetic and Sequence games
...
const ArithmeticChallengeGame = ({ onBack }: { onBack: () => void }) => {
 ...
 resetGame();
  ...
  const handleSubmitAnswer = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!currentProblem || isGameOver || feedback) return; // Prevent submit while feedback is shown
...
interface SequenceProblem {
  id: string;
  sequence: number[]; // The visible part of the sequence
  nextNumber: number; // The correct next number
...
const NumberSequenceGame = ({ onBack }: { onBack: () => void }) => {
