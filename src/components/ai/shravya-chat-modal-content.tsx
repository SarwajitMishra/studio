...
export default function ShravyaChatModalContent() {
  ...
  const speakText = useCallback((text: string, languageCode: string) => {
    ...
    if (languageCode.toLowerCase().startsWith('hi')) {
      ...
      if (!selectedVoice) { // Broader English check
        const anyEnglishVoices = voices.filter(v => v.lang.toLowerCase().startsWith('en'));
        selectedVoice = anyEnglishVoices.find(v => v.name.toLowerCase().includes('female')) || anyEnglishVoices[0];
      }
