...
export default function CrosswordGame({ difficulty, onBack }: CrosswordGameProps) {
    ...
    const createGridFromPuzzle = useCallback((p: CrosswordPuzzle): Grid => {
        ...
        p.words.forEach((word, index) => {
            if (newGrid[word.row] && newGrid[word.row][word.col]) {
    ...
    const restartGame = useCallback(async () => {
        ...
        const availablePuzzles = CROSSWORD_PUZZLES.filter(p => p.difficulty === difficulty);
        ...
        const staticPuzzle = availablePuzzles[Math.floor(Math.random() * availablePuzzles.length)];

        const newGrid = createGridFromPuzzle(staticPuzzle);

        setPuzzle(staticPuzzle);
        setGrid(newGrid);
        setUserGrid(Array(staticPuzzle.gridSize).fill(null).map(() => Array(staticPuzzle.gridSize).fill('')));
        inputRefs.current = Array(staticPuzzle.gridSize).fill(null).map(() => Array(staticPuzzle.gridSize).fill(null));
        setHintsUsed(0);
    ...
    if(isGenerating || !puzzle || grid.length === 0) return (
        <div className="flex flex-col items-center justify-center h-96 w-full max-w-lg">
            ...
            <p className="mt-4 text-muted-foreground">Loading a new crossword...</p>
        </div>
    );
        ...
        <div ref={gameContainerRef} className={cn("w-full h-full flex flex-col lg:flex-row items-start justify-center p-2 md:p-4 gap-6", isFullscreen && "bg-background")}>
            ...
            <div className="w-full lg:w-auto space-y-4">
                 <div className="w-full max-w-md mx-auto p-1 sm:p-2 bg-card rounded-lg shadow-lg">
                    <div className="grid aspect-square" style={{ gridTemplateColumns: `repeat(${puzzle.gridSize}, minmax(0, 1fr))` }}>
                ...
                <Card className="shadow-lg">
                    ...
                    <CardContent className="grid grid-cols-2 gap-2">
                        ...
                        <Button onClick={restartGame} variant="destructive" className="col-span-2"><RotateCw className="mr-2"/>Reset</Button>
                    </CardContent>
                </Card>
